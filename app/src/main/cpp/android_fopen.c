...
FILE* android_fopen(const char* fname, const char* mode) 
{
	if(mode[0] == 'w')
	{
		LOGI("Cannot write to Android assets %s", fname);
		return NULL;
	}
	if(!android_asset_manager)
	{
		LOGI("Manager has not been set. Was android_fopen_set_asset_manager() called?");
		return NULL;
	}
	AAsset* asset = AAssetManager_open(android_asset_manager, fname, 0);
	if(!asset)
	{
		LOGI("Asset '%s' not found by manager.", fname);
		return NULL;
	}
